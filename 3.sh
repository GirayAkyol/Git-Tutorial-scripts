...
while true; do
    ...
    git_log=$(git log --color=always -n 10 --reflog --oneline --all --graph --decorate --tags --remotes --topo-order --format="%C(auto)%h%C(reset) %C(yellow)%d%C(reset) %C(green)%s%C(reset) %C(cyan)(%cr)%C(reset) %C(blue)%b%C(reset) %C(magenta)%+%C(yellow) %-%C(reset)")
